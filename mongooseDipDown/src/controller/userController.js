...
class userController{
    ...
    static async forgotPassword(req,res,next){
        try {
            ...
            const expiredIn = Date.now() + parseInt(config.RESETTOKENEXPIRATION)
            ...
            out(res,200,`Visit your email to reset password. This token will be expired in 5 min. ${resetURL}` )
        } catch (error) {
            next(new appError(500,'ERROR',error))
        }
    }
